...
export function useReflogEntries(
  repoPath: string | null,
  limit?: number,
  enabled = true,
) {
...
export function useHistorySurgeryMutations(repoPath: string | null) {
  ...
  return {
    cherryPick: useMutation(gitMutations.cherryPickCommit(repoPath)),
    revert: useMutation(gitMutations.revertCommit(repoPath)),
    previewReset: useMutation(
      gitMutations.previewResetToCommit(queryClient, repoPath),
    ),
    ...
    checkoutReflogEntry: useMutation(
      gitMutations.checkoutReflogEntry(queryClient, repoPath),
    ),
    createBranchFromReflogEntry: useMutation(
      gitMutations.createBranchFromReflogEntry(queryClient, repoPath),
    ),
  };
}
